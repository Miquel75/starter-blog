...
L'exemple le plus simple avec une seule variable explicative c'est :  
  ` y = a.x + b + e  `  <br/>
y est la variable expliquée (indépendante)    <br/>
x est la variable explicative (dépendante)    <br/>
...
On fait notre régression linéaire entre la profondeur et la taille des poissons de notre échantillon et on visualise le résultat. 

```{r}
#régression linéaire avec fonction lm (?lm pour l'aide R)
mod<-lm(PROF~TAILLE, donnees_na)
#visualiser les données
plot(donnees_na$TAILLE, donnees_na$PROF)
abline(mod)
title("Régression linéaire")
summary(mod)
#visualiser la distance des points par rapport au modèle
segments(donnees_na$TAILLE,fitted(mod),donnees_na$TAILLE, donnees_na$PROF)
#frame valeurs taille
pred.frame<-data.frame(TAILLE= 1.50:41)
...
**Multiple R-squared** -> R² est la proportion de variance expliquée par le modèle. *Plus R² est proche de 1, plus la variabilité des données est expliqué par le modèle.*  Adjusted R-squared est plus précis dans le cas d'une régression multiple. La racine carré de R² donne le coefficient de corrélation.    
